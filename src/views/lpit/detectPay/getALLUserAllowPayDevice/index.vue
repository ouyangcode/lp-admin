          ...
          <template slot-scope="scope">
            ...
            >
              <el-button
                ...
                type="primary"
                icon="el-icon-edit"
                @click="toHiddenList(scope.row,index)"
              >设置为黑名单</el-button>
              ...
              <el-button
                ...
                type="primary"
                icon="el-icon-edit"
                @click="toWhiteList(scope.row,index)"
              >设置为白名单</el-button>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="objKey" align="center" label="设备ID" />
        <el-table-column prop="objVal" align="center" label="信息" />
      </el-table>
      <!--分页组件-->
      <pagination />
    </div>
...
<script>
import {
  lpGetALLUserAllowPayDevice,
  gameCodeList,
  toWhiteOrForbiddenDevice
} from '@/api/lpitmgr/detectPay/getALLUserAllowPayDevice'
...
export default {
  name: 'GetALLUserAllowPayDevice',
  components: { rrOperation, crudOperation, pagination },
  mixins: [presenter(), header(), form(defaultForm), crud()],
  cruds() {
    return CRUD({
      title: 'soho測試生成',
      url: '/api/detectPay/getALLUserAllowPayDevice',
      crudMethod: { ...lpGetALLUserAllowPayDevice }
    })
  },
  data() {
    return {
