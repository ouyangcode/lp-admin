  ...
  <div class="app-container">
    ...
    <div class="head-container">
      ...
      >
        ...
        <el-table-column align="center" prop="totalMoney" label="totalMoney" />
        ...
        <el-table-column align="center" prop="num" label="储值笔数">
          <template slot-scope="scope">
            {{ scope.row.orderIDSet.length }}
          </template>
        </el-table-column>

        <el-table-column prop="itemCodeSet" align="center" label="品项列表">
          <template slot-scope="scope">
            <div
              v-for="(item,index) of scope.row.itemCodeSet"
              :key="index"
              class="open"
            >
              {{ item }}
        ...
        <el-table-column prop="moneySet" align="center" label="金额列表">
          <template slot-scope="scope">
            <div
              v-for="(item,index) of scope.row.moneySet"
              :key="index"
              class="open"
            >
              {{ item }}
        ...
        <el-table-column prop="orderIDSet" align="center" label="订单列表" width="220px">
          <template slot-scope="scope">
            <div
              v-for="(item,index) of scope.row.orderIDSet"
              :key="index"
              class="open"
            >
              {{ item }}
...
<script>
import {
  lpGetUserGooglePayInfo,
  toSetAllowCurrencyPayIOS
} from '@/api/lpitmgr/detectThirdPartyPayOpen/getUserGooglePayInfo'
...
export default {
  name: 'GetUserGooglePayInfo',
  components: { rrOperation, crudOperation, pagination },
  mixins: [presenter(), header(), form(defaultForm), crud()],
  cruds() {
    return CRUD({
      title: 'soho測試生成',
      url: '/api/detectThirdPartyPayOpen/getUserGooglePayInfo',
      crudMethod: { ...lpGetUserGooglePayInfo }
    })
  },
  data() {
    return {
