  ...
  <div class="app-container">
    ...
    <div class="head-container">
      ...
      <el-table ref="table" v-loading="crud.loading" :height="tableHeight" :header-cell-style="{background:'#eef1f6',color:'#606266'}" border :data="crud.data" size="small" style="width: 100%;">
        ...
        <el-table-column prop="date" align="center" label="时间" width="200px" />
        <el-table-column prop="index0" align="center" label="次留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index0">{{ scope.row.index0 + "%" }}</div>

          </template>
        </el-table-column>
        <el-table-column prop="index1" align="center" label="3留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index1">{{ scope.row.index1 + "%" }}</div>

          </template>
        </el-table-column>
        <el-table-column prop="index2" align="center" label="4留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index2">{{ scope.row.index2 + "%" }}</div>

          </template>
        </el-table-column>
        <el-table-column prop="index3" align="center" label="5留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index3">{{ scope.row.index3 + "%" }}</div>

          </template>
        </el-table-column>
        <el-table-column prop="index4" align="center" label="6留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index4">{{ scope.row.index4 + "%" }}</div>

          </template>
        </el-table-column>
        <el-table-column prop="index5" align="center" label="7留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index5">{{ scope.row.index5 + "%" }}</div>

          </template>
        </el-table-column>
        <el-table-column prop="index6" align="center" label="8留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index6"> {{ scope.row.index6 + "%" }}</div>

          </template>
        </el-table-column>
        <el-table-column prop="index7" align="center" label="9留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index7"> {{ scope.row.index7 + "%" }}</div>

          </template>
        </el-table-column>
        <el-table-column prop="index8" align="center" label="10留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index8">{{ scope.row.index8 + "%" }}</div>

          </template>
        </el-table-column>
        <el-table-column prop="index9" align="center" label="11留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index9"> {{ scope.row.index9 + "%" }}</div>

          </template>
        </el-table-column>
        <el-table-column prop="index10" align="center" label="12留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index10">{{ scope.row.index10 + "%" }}</div>

          </template>
        </el-table-column>
        <el-table-column prop="index11" align="center" label="13留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index11">{{ scope.row.index11 + "%" }}</div>

          </template>
        </el-table-column>
        <el-table-column prop="index12" align="center" label="14留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index12">{{ scope.row.index12 + "%" }}</div>

          </template>
        </el-table-column>
        <el-table-column prop="index13" align="center" label="15留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index13">{{ scope.row.index13 + "%" }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="index14" align="center" label="30留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index14">{{ scope.row.index14 + "%" }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="index15" align="center" label="60留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index15"> {{ scope.row.index15 + "%" }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="index16" align="center" label="90留" width="110px">
          <template slot-scope="scope">
            <div v-if="scope.row.index16">{{ scope.row.index16 + "%" }}</div>
          </template>
        </el-table-column>
      </el-table>
      <!--分页组件-->
...
<script>
import { lpGmDateLoseRateReport, gameCodeList, download, todownForPage } from '@/api/lpmain/gmDateRetainedReport'
...
export default {
  ...
  cruds() {
    return CRUD({ title: 'soho測試生成', url: '/api/gmDateRetainedReport/reportList', crudMethod: { ...lpGmDateLoseRateReport }})
  },
  data() {
    return {
      permission: { },
